...
with open('exert_moto.csv','r') as cfofile:
...
with open('times.csv','r') as timefile:
...
with open('room_temp','r') as tempfile:
    csvreader = csv.reader(tempfile,delimiter=',')
    for row in csvreader:
...
ax.set_ylabel(r'CFO(KHz)',fontsize=16)
...
ax.annotate(r'Start Game',xy=(0.1,1.125),xycoords=trans,ha='center',va='top',fontsize=13)
ax.plot([0.1,0.1],[1.0,1.05],transform=trans,color='#581845',linewidth=1.0,linestyle='--',clip_on=False)
ax.annotate(r'Stop Game',xy=(12.0,1.125),xycoords=trans,ha='center',va='top',fontsize=13)
...
ax.tick_params(axis='x', which='minor', right=False)
...
ax3.set_xlim(0,15)
ax3.set_ylim(26,42)
ax3.plot(temp_x,temp,linewidth=2.0,label=r'PMIC temperature',color='#2c7fb8')

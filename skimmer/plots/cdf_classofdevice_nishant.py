...
cur.execute("create table cod_cdf_table as (select distinct on (mac) mac,loc,devmajor,devminor,devtype from enqresp where obsmac = '1af0fd7db5bb53bf' and loc is not null)")
for row in rows:
...
#print categorized
with open('categorized_nishant.json','wb') as file:
	file.write(json.dumps(categorized))

with open('uncategorized_nishant.json','wb') as file:
	file.write(json.dumps(uncategorized))
with open('ble_nishant.json','wb') as file:
